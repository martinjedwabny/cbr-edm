...
class ProbFOIL(LearnEntail):
    ...
    def best_rule(self, current):
                ...
                while candidates:
                    ...
                    if self._max_length and len(current_rule) >= self._max_length:
                        ...
                        for ref in self.language.refine(current_rule):
                            ...
                            if (self._missing_required_predicates(best_rule) and not self._missing_required_predicates(rule)):
                                best_rule = rule
                                c_tp, c_fp, c_tn, c_fn = rates(rule)
                                prev_tp = c_tp
                            elif prev_tp > r_tp - 1e-8:       # new rule has no tp improvement over previous
                                ...
                                if rule.score_cmp > best_rule.score_cmp and not self._missing_required_predicates(rule):
        ...
        while best_rule.parent and not self._missing_required_predicates(best_rule.parent) and best_rule.parent.score > best_rule.score - 1e-8:
            best_rule = best_rule.parent

        self._select_rule(best_rule)
        return best_rule

    def _missing_required_predicates(self, rule):
        functors = set(map(lambda l : str(l.functor), rule.get_literals()))
        requireds = set(map(lambda x : str(x), self.language._requireds))
        return not requireds.issubset(functors)
...
def argparser():
    ...
    parser.add_argument('--symmetry-breaking', action='store_true',
                        help='avoid symmetries in refinement operator', default=True)
